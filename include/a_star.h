...
namespace AStar {
    static bool isValid(const sf::Vector2i& current, const sf::Vector2i& next, const Maze& maze, const vector<vector<bool>>& visited, LevelManager& levelManager);
    int heuristic(const sf::Vector2i& a, const sf::Vector2i& b);
    bool findPath(const Maze& maze, const sf::Vector2i& enemyPos, const sf::Vector2i& playerPos, std::vector<sf::Vector2i>& path, LevelManager& levelManager);
}
